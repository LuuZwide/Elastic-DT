...
def base_parse():
    ...
    parser.add_argument("--project_name", type=str, default="elastic-decision-transformer")
...
def get_d4rl_dataset_stats(env_d4rl_name):
    ...
    obersavations = dataset["observations"]

    state_mean = np.mean(obersavations, axis=0)
    state_std = np.std(obersavations, axis=0) + 1e-6
...
def edt_evaluate(
    ...
    **kwargs
    ...
    return_rewards = torch.zeros(0, device=device, dtype=torch.float32)
    ...
    with torch.no_grad():
        ...
        for _ in range(num_eval_ep):
            ...
            for t in range(max_test_ep_len):
                total_timesteps += 1
                return_rewards = torch.cat([return_rewards, torch.zeros(1, device=device)]) 
...
def eval_checkpoints(grouped):
    ...
    for dataset, ckpts in grouped.items():
        for i, (step, ckpt_path) in enumerate(ckpts):
            ...
            print(f"Evaluating {ckpt_path} -> Table: {table_name}")
